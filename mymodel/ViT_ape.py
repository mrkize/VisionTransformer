...
class VIT(nn.Module):
    def __init__(self, image_size, patch_size, embed_dim, num_classes, depth, heads, mlp_dim, pool='cls', channels=3, dim_head=8, dropout=0., emb_dropout=0.):
        ...
        self.PE = True
        self.pos_embedding = PositionalEncoding(embed_dim).pe[:,:num_patches+1,:]
    ...
    def forward(self, x, unk_mask = None):
        ...
        # self.pos_embedding.to(x.device)
        ...
        if unk_mask is not None:
            ...
            pos = torch.cat([self.pos_embedding, torch.zeros(1, 1, self.embed_dim).to(x.device)], dim=1)
