...
def mask_train_model(opt, config, data_loader, data_size, is_target = True):
    ...
    if config.learning.DDP or config.learning.DP:
        if opt.local_rank == 0:
            torch.save(model.module.state_dict(), '../defence_model/{}/Basic/Basic{}.pth'.format(opt.dataset, pad))
    else:
        torch.save(model.state_dict(), '../defence_model/{}/Basic/Basic{}.pth'.format(opt.dataset, pad))
    return
...
print("Basic dataset : {}".format(opt.dataset))
