    ...
    for epoch in range(config.learning.epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for batch_idx, (data, target) in enumerate(loader[phase]):
                ...
                unk_mask = None
                if phase == 'train':
                    if mixup_fn is not None and epoch >= 20:
...
def mask_train_model(opt, config, data_loader, data_size, is_target = True):
    ...
    if config.learning.DDP or config.learning.DP:
        if opt.local_rank == 0:
            torch.save(model.module.state_dict(), '../defence_model/{}/label_smoothing/ls_{}{}.pth'.format(opt.dataset, opt.ls, pad))
    else:
        torch.save(model.state_dict(), '../defence_model/{}/label_smoothing/ls_{}{}.pth'.format(opt.dataset, opt.ls, pad))
    return
...
print("label_smoothing dataset : {} ls: {}".format(opt.dataset, opt.ls))
